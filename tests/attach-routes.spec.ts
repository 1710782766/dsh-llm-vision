/**
 * Route tests for /llm-vision/attach: payload validation, the
...
describe('handleAttach', () => {
  it('persists a valid image and returns its note', async () => {
    ...
    if (outcome.ok) {
      ...
      expect(outcome.markdown).toMatch(/^!\[图片\]\(\/llm-vision\/raw\/sha256:/)
...
describe('registerAttachRoute', () => {
  ...
  const makeReq = (method: string, body?: string, url = '/llm-vision/attach'): IncomingMessage => ({
    method,
    url,
  ...
  it('registers the prefix route on the webserver', () => {
    ...
    expect(registrations[0].path).toBe('/llm-vision')
  })
