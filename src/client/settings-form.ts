 ...
 * screen exactly what a save would store. Originated as a family-shared
 * slice (mirroring the official ui-plugin-config card-store pattern) and
 * maintained directly in this package since the browser half migrated off
 * the dsh-client-runtime bundle.
 */
...
export interface CardShell {
   ...
   * Host deployment does not serve it (no settings provider, or the owning
   * plugin has not registered its section): the card renders an explanation
