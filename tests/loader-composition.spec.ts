...
async function tempPng(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'dsh-llm-vision-loader-file-'))
...
describe('describe-image real Loader composition through cordis.yml', () => {
  ...
  // baseURL omitted: loads silently (aggregate-friendly lazy mount) — covered
  // by the mount-without-configuration test below.
  it.each([
