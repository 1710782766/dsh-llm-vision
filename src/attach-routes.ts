/**
 * The /llm-vision/attach route: a browser-to-host upload seam that turns a
...
export function attachmentMarkdown(id: string): string {
  ...
  return `![图片](/llm-vision/raw/${encodeURIComponent(id).replace(/%3A/gi, ':')})`
}
...
async function serveRawImage(ctx: Context, req: IncomingMessage, res: ServerResponse): Promise<void> {
  const match = /^\/llm-vision\/raw\/([^/]+)$/.exec(new URL(req.url ?? '/', 'http://x').pathname)
  if (match === null) {
    res.writeHead(404)
    res.end()
    return
...
/**
 * Register the /llm-vision/attach POST route on the shared webserver. The
...
export function registerAttachRoute(ctx: Context, readMaxBytes: () => number = () => DEFAULT_MAX_BYTES): void {
  ...
  webserver.register({
    ...
    handler: async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      // GET /llm-vision/raw/<id>: serve the stored bytes so the
