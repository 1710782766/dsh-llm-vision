...
const SPEC: tool.ResolvedConfig = tool.resolveConfig({
...
async function boot(ctx: Context, baseURL: string, cacheDir?: string): Promise<void> {
  await ctx.plugin(FakeWebServer)
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(tool, { baseURL, model: 'vision-1', apiKey: 'sk-inline', ...cacheDir === undefined ? {} : { cacheDir } })
}
