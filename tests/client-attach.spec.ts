...
describe('uploadImageForDescribe', () => {
  it('posts base64, type, and name and returns the note', async () => {
    const fetchMock = vi.fn(async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({ ok: true, value: { note: '[image attachment {}]', markdown: '![图片](/llm-vision/raw/sha256:x)' } }), { status: 200 })
    })
    vi.stubGlobal('fetch', fetchMock)
    const outcome = await uploadImageForDescribe('QUJD', 'image/png', 'pic.png')
    expect(outcome).toEqual({ ok: true, note: '[image attachment {}]', markdown: '![图片](/llm-vision/raw/sha256:x)' })
    expect(fetchMock).toHaveBeenCalledWith('/llm-vision/attach', expect.objectContaining({ method: 'POST' }))
  })
