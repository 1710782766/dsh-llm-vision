 ...
 * send hook splices (`![图片](/llm-vision/raw/sha256:…)`) sits in the
 ...
 * @module dsh-llm-vision/client/preview
 */
...
const REFERENCE_PATTERN = /!\[([^\]]*)]\((\/llm-vision\/raw\/[^)\s]+)\)/g
...
export function installConversationImagePreview(isEnabled: () => boolean, root?: HTMLElement): ConversationImagePreview {
  ...
  const scanNode = (node: Node): void => {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (text.data.includes('/llm-vision/raw/') && !isExcluded(text)) enhanceNode(text)
      return
    }
    ...
    const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, {
      acceptNode: candidate => {
        ...
        if (!text.data.includes('/llm-vision/raw/')) return NodeFilter.FILTER_REJECT
