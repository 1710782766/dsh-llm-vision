...
describe('installSendHook', () => {
  ...
  it('rewrites an image-bearing send into a text prompt carrying the reference', async () => {
    stubFileReader('QUJD')
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({ ok: true, value: { note: 'N', markdown: '![图片](/llm-vision/raw/sha256:x)' } }), { status: 200 })))
    const { face, log } = makeConversation()
    ...
    expect(blocks[0].text).toContain('![图片](/llm-vision/raw/sha256:x)')
  })
